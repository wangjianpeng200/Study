//有向图，并且带权值
/*
创建两个集合，一个是已访问顶点集合，另一个是未访问顶点集合。开始时，已访问集合只包含起点，所有其他顶点都在未访问集合中。
设置一个距离数组，数组的每个元素代表起点到图中某顶点的最短距离。开始时，起点到自身的距离设为0，到所有其他顶点的距离设为无穷大。
从未访问集合中选出距离最小的顶点，将其加入已访问集合，并从未访问集合中移除。
更新已访问集合中新加入的顶点的所有相邻顶点的距离值。如果通过新加入的顶点到某个相邻顶点的距离小于之前的距离，就用新的距离值替换原来的距离值。
重复步骤3和步骤4，直到所有顶点都加入到已访问集合中，或者找到目标顶点并确定最短路径。
最后，距离数组就代表了从起点开始到所有其他顶点的最短距离，通过反向追踪，我们可以找出最短路径。
*/
...
public:
    ...
    void ShortestPath(T ver,int *dist,int *path);
    void dijkstra();
};
...
void GraphMtx::ShortestPath(T ver,int *dist,int *path)
{
    int n=this->NumVertices;
    bool *S=new bool[n];//标志位
    assert(S!=NULL);
    int v=GetVertexPos(ver);//获取顶点

    //初始化
    for(int i=0;i<this->NumVertices;i++)
    {
        dist[i]=this->GetWeight(v,i);
        S[i]=false;
    ...
    for(int i=0;i<n-1;i++)//寻找最小值
    {
        ...
        S[u]=true;
        //此时u是新的顶点,更改path和dist
        for(int k=0;k<n;k++)
        {
            int w=this->GetWeight(u,k);
...
void GraphMtx::dijkstra()
{
    int n=this->NumVertices;
    int *path=new int[n];
    int *dist=new int[n];
    assert(dist!=NULL && path!=NULL);
    this->ShortestPath('A',dist,path);
    int v=this->GetVertexPos('C');
    cout<<path[v]<<path[3]<<endl;

    cout<<"到达C的最短路径是:";
    while(path[v]!=-1)
    {
        cout<<this->VerticesList[v]<<"<--";
        v=path[v];
    }
    cout<<this->VerticesList[0];
}
void test_dijkstra()
{
    GraphMtx graph;
